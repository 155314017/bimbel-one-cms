        ...
        <table className="table-auto w-full border">
          ...
          <tbody>
            {currentData.map((data, index) => (
              <tr key={index}>
                <td className="border capitalize p-2 text-center text-[12px]">{data.id}</td>
                <td className="border capitalize p-2 text-[12px] bg-slate-100">{data.name}</td>
...
const Pagination = ({ dataPerPage, totalData, paginate, currentPage }: PaginationProps) => {
  // const pageNumbers = [];
  ...
  return (
    <nav className="mt-4 flex justify-between items-center">
      <h1 className="text-[14px] text-slate-400">Showing {currentPage * dataPerPage - dataPerPage + 1} to {currentPage * dataPerPage} of {totalData}</h1>
